...
void Timer::timeStep() {
    uint8_t tmrVal = memoryInterface->readRegister(MemoryInterface::TMR0, 0); // Read TMR0 to trigger any side effects
    tmrVal += prescaler->step(Prescaler::TMR0);
    memoryInterface->writeRegister(MemoryInterface::TMR0, tmrVal, 0);
}
