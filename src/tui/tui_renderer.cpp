              ...
              std::vector<HitBox>& hitBoxes) const
{
    ...
    const std::string status = TUI_Helper::getStatus(state);
    printWithColor(32, 0, TUI_Helper::statusPairFromText(status), A_BOLD, "Status: %s", status.c_str());
}


void TUI_Renderer::drawTopBits(const PICSnapshot& snapshot, std::vector<HitBox>& hitBoxes)
{
...
void TUI_Renderer::drawMemoryGrid(const PICSnapshot& snapshot, std::vector<HitBox>& hitBoxes, const std::optional<uint8_t>& selectedRamAddress)
{
...
void TUI_Renderer::drawControlPanel(const SimulationState& state, std::vector<HitBox>& hitBoxes)
{
...
void TUI_Renderer::printWithColor(int y, int x, short pair, int attrs, const char* format, ...)
{
    int flags = attrs;
    if (1)
    {
