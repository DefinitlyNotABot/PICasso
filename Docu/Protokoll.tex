...
\begin{titlepage}
    \centering
    {\LARGE Programmentwurf - Protokoll\par}
    \vspace{0.5cm}
    {\Large PICasso -- PIC Microcontroller Simulator\par}
    \vfill
    {\large Name: Wachsmann, Nik\par}
    \vspace{0.2cm}
    {\large Matrikelnummer: [MNR]\par}
    \vspace{0.5cm}
    {\large Name: Stahl, Ben\par}
    \vspace{0.2cm}
    {\large Matrikelnummer: 8238700\par}
    \vspace{0.5cm}
    {\large Abgabedatum: 04.05.2026\par}
    \vfill
\end{titlepage}

\section{Kapitel 1: Einführung (4P)}

\subsection{Übersicht über die Applikation (1P)}

\textbf{PICasso} ist ein Simulator für PIC-Midrange-Mikrocontroller (z.B. PIC16F84), geschrieben in C++17. Die Applikation liest assemblierte PIC-Programme im \code{.LST}-Format ein und führt diese Instruktion für Instruktion aus. Dabei werden alle wesentlichen Hardwarekomponenten des PIC simuliert: ein 2-Bank-RAM mit Registerspiegelung, ein 8-Bit-W-Akkumulator, ein 8-Level-Hardware-Stack, ein Timer/Prescaler-Subsystem sowie die vollständige Ausführung aller 35 PIC-Midrange-Instruktionen.
...
\textbf{Zweck:} PICasso löst das Problem, PIC-Assemblerprogramme ohne physische Hardware testen und debuggen zu können. Es dient als Lern- und Entwicklungswerkzeug für eingebettete Systeme.
...
Nach dem Start öffnet sich die ncurses-Oberfläche. Über den \emph{Load}-Button kann eine \code{.LST}-Datei aus dem \code{progs/}-Verzeichnis geladen werden. Anschließend kann das Programm mit \emph{Step}, \emph{Run} (Dauerlauf) oder \emph{Dash} (Schnelllauf) ausgeführt werden.
...
\hline
C++17 & Primäre Programmiersprache & Bietet hardwarenahe Kontrolle, geeignet für Simulator-Entwicklung; unterstützt \code{std::optional}, \code{std::shared_ptr}, \code{std::filesystem} und strukturierte Bindings. \\
\hline
CMake & Build-System & Plattformunabhängiges Build-System; ermöglicht automatisches Herunterladen von Abhängigkeiten (Catch2) via \code{FetchContent}. \\
\hline
...
\hline
Catch2 & Unit-Test-Framework & Header-only-Framework mit BDD-Stil (\code{TEST_CASE}/\code{SECTION}); automatische Testregistrierung und Generatoren (\code{GENERATE}). \\
\hline
...
\hline
\code{std::thread / std::mutex / std::atomic} & Multithreading & Simulation und UI laufen parallel; atomare Flags steuern den Simulationszustand. \\
\hline
\end{longtable}

\section{Kapitel 2: Softwarearchitektur (8P)}

\subsection{Clean Architecture (4P)}

Die Applikation orientiert sich an einer geschichteten Architektur, die Elemente der Clean Architecture aufgreift:

\begin{itemize}
    \item \textbf{Domain / Entities (innerster Ring):} Die Memory-Abstraktion (\code{Bit}, \code{Nibble}, \code{Byte}, \code{Register}) sowie \code{Ram}, \code{Stack} und \code{MemoryInterface}. Diese Klassen modellieren die Hardwaredomäne des PIC-Mikrocontrollers und haben keine Abhängigkeiten zu Framework- oder UI-Code.
    \item \textbf{Use Cases / Application Layer:} \code{Instruction} (mit allen 35+ Unterklassen), \code{ALU}, \code{Compiler}, \code{Program}, \code{Timer}, \code{Prescaler} und \code{PIC}. Diese Schicht orchestriert die Simulation und nutzt die Domain-Klassen.
    \item \textbf{Interface Adapters:} \code{Filereader} (Dateisystem-Zugriff), \code{Logger} (Ausgabe-Abstraktion).
    \item \textbf{Frameworks / UI (äußerster Ring):} \code{TerminalUI}, \code{TUI_Renderer}, \code{TUI_Controller} und alle TUI-Hilfsklassen. Diese hängen von ncurses ab und nutzen \code{PICSnapshot} als Daten-Transfer-Objekt.
\end{itemize}

\umlplaceholder{UML-Klassendiagramm: Schichten der Architektur mit Bit$\rightarrow$Nibble$\rightarrow$Byte$\rightarrow$Register$\rightarrow$Ram$\rightarrow$MemoryInterface$\rightarrow$PIC$\rightarrow$TerminalUI}
...
\textbf{Domain Code} ist Code, der die fachliche Logik der Anwendungsdomäne abbildet -- unabhängig von Frameworks, Datenbanken oder UI. Im Fall von PICasso ist die Domäne die PIC-Mikrocontroller-Hardware.
...
\begin{lstlisting}[style=codeblock,language=C++]
class Stack {
    private:
        uint8_t stack[8];
...
Die Klasse \code{Register} (Domain/Entity) hat keine Abhängigkeiten zu äußeren Schichten. Sie hängt nur von \code{Byte} ab, welches wiederum nur von \code{Nibble} und \code{Bit} abhängt -- alles innerhalb der Domain-Schicht. Die äußere Schicht \code{Ram} hängt von \code{Register} ab, aber \code{Register} weiß nichts von \code{Ram}. Die Abhängigkeit zeigt korrekt von außen nach innen.

\umlplaceholder{UML: Register $\rightarrow$ Byte $\rightarrow$ Nibble $\rightarrow$ Bit (reine Domain-Schicht, keine Rückabhängigkeit)}

\begin{center}
    \includegraphics[width=0.9\textwidth]{UML/2.png}
\end{center}

\subsubsection{Negativ-Beispiel: Dependency Rule}

Die abstrakte Klasse \code{Instruction} (Application Layer) hat ein \textbf{statisches} Feld \code{static std::shared_ptr<MemoryInterface> memoryInterface}, das durch die \code{PIC}-Klasse gesetzt wird. Das bedeutet, dass die Application-Layer-Klasse \code{Instruction} direkt an die konkrete Implementierung \code{MemoryInterface} gekoppelt ist. Zudem wird das statische Feld von \code{PIC} (einem Orchestrator) gesetzt -- eine implizite Abhängigkeit von außen nach innen, die zur Laufzeit besteht, obwohl die Instruction-Klasse eigentlich keine Kenntnis über PIC haben sollte.
...
\umlplaceholder{UML: Instruction --static$\rightarrow$ MemoryInterface $\leftarrow$setzt-- PIC (Verletzung: zirkuläre implizite Abhängigkeit)}

\begin{center}
    \includegraphics[width=0.9\textwidth]{UML/3.png}
\end{center}

\section{Kapitel 3: SOLID (8P)}

\subsection{Analyse SRP (3P)}

\emph{[Jeweils eine Klasse als positives und negatives Beispiel für SRP; jeweils UML und Beschreibung der Aufgabe bzw. der Aufgaben und möglicher Lösungsweg des Negativ-Beispiels (inkl. UML)]}
...
\umlplaceholder{UML: Stack mit Attributen stack[8], stackPointer und Methoden push(), pop(), reset()}
...
\umlplaceholder{UML: PIC (vorher) $\rightarrow$ aufgeteilt in PICHardware, PICSimulator, PICDebugInterface (nachher)}
...
\emph{[Jeweils eine Klasse als positives und negatives Beispiel für OCP]}
...
\umlplaceholder{UML: Instruction (abstract) $\triangleleft$-- Arithmetic $\triangleleft$-- ADDWF, SUBWF, ... | Instruction $\triangleleft$-- Bitwise $\triangleleft$-- BCF, BSF, ...}

\begin{center}
    \includegraphics[width=0.9\textwidth]{UML/6.png}
\end{center}
...
\umlplaceholder{UML: Compiler --verwendet$\rightarrow$ InstructionRegistry --erzeugt$\rightarrow$ Instruction-Unterklassen}

\subsection{Analyse DIP (2P)}

\emph{[Dependency Inversion Principle: jeweils eine Klasse als positives und negatives Beispiel]}
...
Die ALU hängt von der Abstraktion (\code{Instruction}) ab, nicht von den Details (\code{ADDWF}, \code{SUBWF}, etc.) -- konform mit DIP.

\subsubsection{Negativ-Beispiel}

\textbf{Klasse: \code{Instruction}} (alle Unterklassen)

Alle Instruktionen hängen direkt von der konkreten Klasse \code{MemoryInterface} ab (über ein statisches Feld). Es gibt kein Interface oder abstrakte Klasse, gegen die programmiert wird. Falls die Speicherverwaltung anders implementiert werden soll (z.B. für Tests mit Mock-Speicher), muss die konkrete \code{MemoryInterface}-Klasse geändert werden.
...
\textbf{Lösung:} Ein abstraktes Interface \code{IMemoryInterface} einführen, von dem \code{MemoryInterface} erbt. Die \code{Instruction}-Klasse hängt dann nur vom Interface ab:
...
\umlplaceholder{UML: Instruction $\rightarrow$ IMemoryInterface (abstract) $\triangleleft$-- MemoryInterface (concrete)}

\section{Kapitel 4: Weitere Prinzipien (8P)}
...
\umlplaceholder{UML: Program --verwendet$\rightarrow$ Filereader --verwendet$\rightarrow$ std::ifstream (Standardbibliothek)}

\subsection{Analyse GRASP: Pure Fabrication (3P)}
...
\umlplaceholder{UML: Logger (Pure Fabrication, wird von PIC, Compiler, Arithmetic, etc. verwendet)}
...
\textbf{Beispiel:} Die Klasse \code{Arithmetic} (\code{header/instructionset/arithmetic.hpp}) bündelt gemeinsame Logik aller arithmetischen Instruktionen.

\textbf{Vorher (duplizierter Code):} Ohne die gemeinsame Basisklasse müsste jede arithmetische Instruktion (ADDWF, SUBWF, DECF, INCF, etc.) selbst:

\begin{itemize}
    \item den Destination-Bit und die File-Adresse aus dem Opcode extrahieren
    \item den aktuellen W- und File-Wert laden
    \item das Ergebnis je nach Destination zurückschreiben
    \item das Zero-Flag setzen
\end{itemize}
...
\textbf{Nachher (DRY durch Basisklasse):}
...
\textbf{Auswirkung:} Die gemeinsame Logik wird in der Basisklasse \code{Arithmetic} zentralisiert. Änderungen (z.B. am Speicherzugriff) müssen nur einmal vorgenommen werden, nicht in jeder der etwa 11 arithmetischen Instruktionen. Dies reduziert Fehlerquellen und verbessert die Wartbarkeit.

\section{Kapitel 5: Unit Tests (8P)}
...
Alle Tests laufen vollautomatisch über das Catch2-Framework. Mit dem Befehl \code{./AllTests} werden alle Tests ausgeführt, ohne manuelle Konfiguration. Das CMake-Build-System registriert die Tests automatisch via \code{catch_discover_tests()} in der \code{CTestTestfile.cmake}, sodass auch \code{ctest} verwendet werden kann. Keine manuellen Schritte, Eingaben oder Inspektionen sind nötig -- jeder Test besteht oder schlägt fehl mit klarer Fehlermeldung.
...
\emph{[Analyse und Begründung des Einsatzes von 2 Fake/Mock-Objekten; ohne Dritthersteller-Bibliothek; jeweils UML-Diagramm]}
...
\umlplaceholder{UML: FakeProgram (gleiche Schnittstelle wie Program) | PIC $\rightarrow$ Program (austauschbar durch FakeProgram)}

\section{Kapitel 6: Domain Driven Design (8P)}
...
\umlplaceholder{UML: MemoryInterface (Aggregate Root) $\blacklozenge$-- Ram, $\blacklozenge$-- Stack}
...
\section{Kapitel 7: Refactoring (8P)}
...
\section{Kapitel 8: Entwurfsmuster (8P)}

\subsection{Entwurfsmuster: Composite Pattern (4P)}
...
\subsection{Entwurfsmuster: Strategy Pattern / Template Method (4P)}
